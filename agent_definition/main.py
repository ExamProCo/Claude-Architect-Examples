...
RESEARCHER = AgentDefinition(
    # Required: one-line description used by the coordinator to pick this agent
    description="Researches topics and summarizes key facts.",
    # Required: the system prompt that defines the agent's persona and behavior
    prompt="You are a research assistant. When given a topic, list 3 concise bullet-point facts about it. Be brief.",
    # Optional: list of built-in tools the agent can use
    # Available: Read, Write, Edit, Bash, Glob, Grep, WebSearch, WebFetch, AskUserQuestion, Agent
    tools=["WebSearch"],
    # Optional: model override for this specific agent
    # Choices: "sonnet" | "opus" | "haiku" | "inherit" (default: inherit from parent)
    model="inherit",
)
