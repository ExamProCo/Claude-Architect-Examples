...
Education: B.Sc. Computer Science"""
        ...
        print("The only difference is the coordinator system prompt.")
        print("Watch which angles each one thinks to cover.")

        narrow_qs = await run_coordinator(
            client, NARROW_COORDINATOR,
            "NARROW COORDINATOR  (fixed checklist, no gap-checking)",
            job_posting, resume,
        )

        better_qs = await run_coordinator(
            client, BETTER_COORDINATOR,
            "BETTER COORDINATOR  (self-reflection + domain checklist)",
            job_posting, resume,
        )
